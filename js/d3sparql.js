...
*/
d3sparql.htmltable = function(json, config) {   // RY: ADDED config
  var head = json.head.vars
  var data = json.results.bindings
  var opts = {
    "position":  config.position  || "body",
  }
  //d3.select(opts.position).html("");
  var table = d3.select(opts.position).html("").append("table").attr("class", "table table-bordered")
  ...
  thead.append("tr")
    ...
    .enter()
    .append("th")
    .text(function(col) {return col.split("_").join(" ")}) // RY: REPLACE UNDERBARS WITH SPACES
  ...
  table.selectAll("th").style({
    "background": "#eeeeee",
    "text-transform": "capitalize",
    "text-align": "center", // RY: DOES NOT WORK?
  })
}
...
d3sparql.htmlhash = function(json, config) {
  ...
  var table = d3.select(opts.position).append("table").attr("class", "table table-bordered")
  var tbody = table.append("tbody")
  var row = tbody.selectAll("tr")
    ...
    .append("tr")
  row.append("th")
    .text(function(d) {return d.head.split("_").join(" ")}) // RY: REPLACE UNDERBARS WITH SPACES
  row.append("td")
...
d3sparql.barchart = function(json, config) {
  ...
  var svg = d3.select(opts.position)
    .append("svg")
...
d3sparql.piechart = function(json, config) {
  ...
  var svg = d3.select(opts.position) // RY: ORIGINALLY body WAS SELECTED.
    .append("svg")
    .attr("width", opts.width)
    .attr("height", opts.height)
  ...
  var g = gg.selectAll(".arc")
    ...
    .on("mouseover", function(d) {
      ...
      .text(d.data[opts.label].value)
    })
    .on("mouseout", function(d) {
      gg.select("text").remove()
    })
...
d3sparql.treemapzoom = function(json, config) {
  ...
  // of sibling was laid out in ***, we must rescale to fit using absolute
